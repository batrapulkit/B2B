...
import { useClerkFetch } from "@/lib/clerkFetch"; // ✅ import the new hook

export default function Dashboard() {
  const { clerkFetch } = useClerkFetch(); // ✅ get authenticated fetcher

  // ✅ Secure API call with Clerk token
  const { data: analytics, isLoading: analyticsLoading, error: analyticsError } = useQuery({
    queryKey: ["/api/analytics/overview"],
    queryFn: () => clerkFetch("/api/analytics/overview"),
  });

  const { data: itineraries, isLoading: itinerariesLoading, error: itinerariesError } = useQuery({
    queryKey: ["/api/itineraries"],
    queryFn: () => clerkFetch("/api/itineraries"),
  });
    ...
    <div className="space-y-8">
      ...
      <div>
        <div className="flex items-center justify-between mb-6">
          ...
          <Button variant="outline" data-testid="button-view-all-itineraries">
            View All
          </Button>
        </div>
        ...
        {itinerariesLoading ? (
          ...
          <div className="text-red-500">Failed to load itineraries.</div>
